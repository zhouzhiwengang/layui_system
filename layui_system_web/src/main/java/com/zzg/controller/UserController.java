...
import com.zzg.sercurity.util.JwtTokenUtil;
import com.zzg.service.UcasAuthUserInfoService;
...
public class UserController extends AbstractController {
	...
	public Result update(@RequestBody UcasAuthUserInfo entity) {
		boolean target = ucasAuthUserInfoService.saveOrUpdate(entity);
		if (target) {
			return Result.ok();
		} else {
	...
	@ApiOperation(httpMethod = "POST", value = "用户更新密码")
	...
	public Result updateAccepState(@RequestBody Map<String, Object> parame) {
		...
		UpdateWrapper<UcasAuthUserInfo> updateWrapper = new UpdateWrapper<>();
		updateWrapper.eq("sid",String.valueOf(parame.get("sid")));

		UcasAuthUserInfo entity = new UcasAuthUserInfo();
		entity.setPassword(String.valueOf(parame.get("password")));
		
		boolean target = ucasAuthUserInfoService.saveOrUpdate(entity, updateWrapper);
		if (target) {
			return Result.ok();
		} else {
	...
	// 查
	@ApiOperation(httpMethod = "POST", value = "查询当前用户名称")
	@RequestMapping(value = "/getCurrentUser", method = { RequestMethod.POST })
	public Result getCurrentUser(@RequestHeader("token") String token) {
		String username = JwtTokenUtil.getUsername(token);
		if(StringUtils.isNotEmpty(username)){
			QueryWrapper<UcasAuthUserInfo> query = new QueryWrapper<UcasAuthUserInfo>();
			query.eq("user_pin", username);
			return Result.ok().setDatas(ucasAuthUserInfoService.getOne(query));
		}
		return Result.error("未查询到当前用户信息");
	}
